...
# from Spot_Micro_Control_v01 import simulate
# simulate = True
...
class Spot():
    ...
    def __init__(self, simulate=True):
        if not simulate:self.kit = ServoKit(channels=16, address=0x40)
        if not simulate:self.kit2 = ServoKit(channels=16, address=0x41)
        ...
        self.angle_scale_factor_lf1 =  5/4 #足
        self.angle_scale_factor_lf2 =  4/3 #臂
        self.angle_scale_factor_lf3 =  3/3 #肩

        self.angle_scale_factor_rf1 =  5/4 #足
        self.angle_scale_factor_rf2 =  4/3 #臂
        self.angle_scale_factor_rf3 =  3/3 #肩

        self.angle_scale_factor_rr1 =  4/3 #足
        self.angle_scale_factor_rr2 =  4/3 #臂
        self.angle_scale_factor_rr3 =  3/3 #肩

        self.angle_scale_factor_lr1 =  4/3 #足
        self.angle_scale_factor_lr2 =  4/3 #臂
        self.angle_scale_factor_lr3 =  3/3 #肩
    ...
    def servo(self, channel, angle_list):
        for angle in angle_list:
            self.kit2.servo[int(channel-1)].angle = angle
            time.sleep(0.5)
    ...
    def modservo2(self, channel, degree, joint):
        factor_ = eval('self.angle_scale_factor_%s' % joint)
        dir_ = eval('self.dir_%s' % joint)
        zero_ = eval('self.zero_%s' % joint)
        real_angle = degree / pi * 180 * factor_ *  dir_ + zero_
        try:
            self.kit2.servo[channel-1].angle = real_angle
        except ValueError:
            if real_angle >= 180:
                print("%s Angle out of range, set angle to 180" % joint)
                self.kit2.servo[channel-1].angle = 180
            elif real_angle <= 0:
                print("%s Angle out of range, set angle to 0" % joint)
                self.kit2.servo[channel-1].angle = 0
