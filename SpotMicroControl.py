...
"""
import sys 
simulate = True if sys.argv[1] == 'True' else False
BuleConnect = True if sys.argv[2] == 'True' else False
...
Spot = Spotmicro_Inverse_Kinematics_and_Position_Library_v01.Spot(simulate)
...
try:
    ...
    print('Please Connect XBOX ONE to Pi....If you connect just Ignore....')
    exit()
joystick.init()
...
def moving (pos,move): 
    thetalf_reply = Spot.IK(Spot.L0, Spot.L1, Spot.L2, Spot.d, pos[0], pos[1], pos[2], 1)
    thetarf_reply = Spot.IK(Spot.L0, Spot.L1, Spot.L2, Spot.d, pos[3], pos[4], pos[5], -1)
    thetarr_reply = Spot.IK(Spot.L0, Spot.L1, Spot.L2, Spot.d, pos[6], pos[7], pos[8], -1)
    thetalr_reply = Spot.IK(Spot.L0, Spot.L1, Spot.L2, Spot.d, pos[9], pos[10], pos[11], 1)
...
while (continuer):
        ...
        ************************************************************************************************ 
        ...
        if not simulate:
            # 動作
            Spot.modservo2(9, thetalf[2], 'lf1')
            Spot.modservo2(10, thetalf[1], 'lf2')
            Spot.modservo2(11, thetalf[0], 'lf3')

            Spot.modservo2(5, thetarf[2], 'rf1')
            Spot.modservo2(6, thetarf[1], 'rf2')
            Spot.modservo2(7, thetarf[0], 'rf3')

            Spot.modservo(1, thetarr[2], 'rr1')
            Spot.modservo(2, thetarr[1], 'rr2')
            Spot.modservo(3, thetarr[0], 'rr3')

            Spot.modservo(13, thetalr[2], 'lr1')
            Spot.modservo(14, thetalr[1], 'lr2')
            Spot.modservo(15, thetalr[0], 'lr3')
        ...
        # print('===============================')
        # print("左前腳: ", [round(i, 2) for i in thetalf])
        # print("右前腳: ", [round(i, 2) for i in thetarf])
        # print("左後腳: ", [round(i, 2) for i in thetalr])
        # print("右後腳: ", [round(i, 2) for i in thetarr])
        # print('===============================')
        # print('===============================')
